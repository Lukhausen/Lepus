...
Im Rahmen moderner KI-Anwendungen lassen sich zwei zentrale Phasen unterscheiden: Zum einen der Trainingsprozess, zum anderen die Inferenz. Während des Trainings, oft als „Train-Time Compute“ 
// Eher Train Time
bezeichnet, wird ein Modell mithilfe großer Datenmengen und spezieller Optimierungsverfahren angepasst. Hierfür sind umfangreiche Rechenressourcen erforderlich, da jedes Eingabebeispiel den Algorithmus zunächst in einem Vorwärtsdurchlauf durchläuft, bevor der sogenannte Backpropagation-Schritt erfolgt. Dieser Prozess dient dazu, die Parameter so zu verändern, dass das Modell immer zuverlässigere Vorhersagen treffen kann. Je nach Komplexität und Größe des Netzwerks kann das Training sehr viel Zeit in Anspruch nehmen und geht üblicherweise mit hohen Kosten für Hardware und Energie einher.

Sobald das Modell ausreichend gut trainert ist, wird es in den Einsatz überführt. Diese praktische Anwendung bezeichnet man als Inferenz oder „Test-Time Compute“. Hier geht es ausschließlich darum, die gelernten Parameter abzurufen und auf neue Eingaben anzuwenden. Anders als beim Training wird das Modell während der Inferenz nicht mehr verändert, sondern nutzt die zuvor gelernten Zusammenhänge, um Vorhersagen oder Entscheidungen zu treffen. Inference ist zwar in der Regel deutlich schneller und weniger rechenintensiv als das Training, er kann jedoch je nach Modellgröße, Einsatzumgebung und Anzahl der Anfragen trotzdem Ressourcen beanspruchen. In produktiven Szenarien ist daher eine effiziente Inferenz ebenso wichtig wie ein gut organisierter Trainingsprozess, weil das eingesetzte KI-System oftmals große Mengen an Anfragen in kurzer Zeit bewältigen muss.

= Verschiedene Modellarchitekturen und ihr Rechenaufwand

In der Welt der KI existieren diverse Modellarchitekturen, die jeweils ihren eigenen Rechenaufwand mit sich bringen und damit sowohl das Training als auch die Inferenz unterschiedlich beeinflussen. Transformers wie GPT, BERT oder T5 haben sich in letzter Zeit insbesondere in der Sprachverarbeitung etabliert. Ihre Attention - Mechanismen ermöglichen es, kontextbezogene Informationen über lange Sequenzen hinweg effizient zu erfassen, was allerdings im Training große Ressourcen erfordert.
//Nur durch die Attention konnten das Training Parallelisiert werden und dadurch war das verständiss und die verarbertiung von unglaublich großen Input daten möglich.. 
Sobald sie jedoch einmal daraufhin optimiert wurden, umfangreiche Texte und komplexe Zusammenhänge zu verstehen, arbeiten sie in der Inferenz meist trotz ihrer hohen Parametermengen relativ stabil, sofern geeignete Hardware (zum Beispiel GPUs) verfügbar ist.
//Probelm: Bisher wurden modelle immer größer und größer weil das das einzige war, das wir machen konnten um sie besser zu amchen

RNNs und LSTMs waren früher die dominierenden Architekturen im Bereich natürlicher Sprachverarbeitung, ehe die Transformer-Modelle aufkamen. Sie verarbeiten Daten seriell, was oft zu längeren Trainingszeiten führt, wenn es um sehr lange Sequenzen geht. Durch das schrittweise Voranschreiten in jedem Zeitschritt entsteht während des Trainings ein größerer Aufwand, der sich in einer häufig vergleichsweise langsameren Konvergenz äußert.
//Stimmt das da unten? Wenn ja welche Quelle? 
Für die Anwendung selbst benötigt ein ausgereiftes RNN oder LSTM im Allgemeinen weniger Leistung als Transformers, ist aber bei sehr komplexen Aufgaben oft nicht so leistungsstark.

//Was für Bildbereich? Vision Erkennung? Bildgenerierung? What?
...
// Also das ist basucally einfach wie man das Lernen einstellt. Entweder im Training oder im fine tuning.

//Deep learning? Das kennen wir bisher noch nicht / wurde nicht vorher erklärt. 
...
//Jetzt vlt noch dazu packen, das das Relevant ist weil wir ja ein model fine tunen und da genau die gleichen Einstellungen haben die wir bestmöglich wählen müssen

= Test Time Compute
//Fühlt sich etwas doppelt an, weil du ganz am anfang schon mal Train und test time compute hattest
...
// Bro.. can we please write in english xD All of this sounds so wild in German xD FP8 For the win xD


Welches ist besser?
//Welches was? Welches Modell? Welches Test time / Train time? Wir haben noch nicht mal Train time gehabt, das kommt irgendwei nochmal unter dem nächsten absatz
...
//Sachen die hier reingeworfen werden sollten wir auch erklären, wenigstens in einem satz. Was ist Modellquantisierung was ist pruning? :D
...
// Würde vlt erst Train time und dann test time erklären weil test time darauf basiert, dass wir schon mal train time hatten xD

----- welches ist besser?
//Same Here.. Was welches? xD
...
// Das ist irgendwie nochmal das gleich aber anderst geschwafelt
...
//Bro. jetzt fängst du hart an dich zu wiederholen xD
...
//Chain of THought ist an sich keine funkionalität sodnern ne promping technik. 
...
//ALter...Schreib das mal sleber und lass das nicht alles von Chatgpt Schreiben!! xD Das stinkt bis an den arsch der welt und noch drei dörfer weiter nach Chatgpt 
//und bei cot gehts nicht darum das der user das besser nachvollziehen kann xD
