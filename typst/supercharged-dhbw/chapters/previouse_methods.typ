...
+ *Depth-First Search (DFS) candidate generation*: Traditional token selection employs greedy decoding, where models select the highest-probability token at each step without considering future implications. In contrast, this DFS approach branches out by following several high-probability candidate tokens simultaneously, exploring complete reasoning paths to their conclusion. Each path's total probability is calculated by multiplying token probabilities along the entire sequence. The system then selects the branch with the highest cumulative confidence, allowing slightly less probable intermediate tokens to succeed if they lead to more confident final conclusions.
