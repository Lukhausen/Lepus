...
if [ ! -d "TinyZero" ]; then
    git clone https://github.com/Jiayi-Pan/TinyZero.git > /dev/null
else
...
pip install wandb
pip install -e . # Installs 'verl' from local source
...
echo "[5/6] Downloading Qwen2.5-0.5B model (if needed)..."
mkdir -p models/
# Check if model directory exists before cloning
if [ ! -d "models/Qwen2.5-0.5B/.git" ]; then # Check for .git to be more robust
    cd models/
    echo "    Cloning model repo (this uses LFS and may take time)..."
    # Clone the model - Git LFS is essential here
    git lfs clone https://huggingface.co/Qwen/Qwen2.5-0.5B # Keep output for progress
    cd ../ # Go back to TinyZero project root
    echo "    Model download complete."
else
    echo "    Model directory models/Qwen2.5-0.5B already exists."
fi
...
echo "   python ./examples/data_preprocess/countdown.py --local_dir ./data/countdown"
echo ""
echo "C. Set Environment Variables (while 'zero' env is active):"
echo "   export N_GPUS=1"
echo "   export BASE_MODEL=./models/Qwen2.5-0.5B"
echo "   export DATA_DIR=./data/countdown"
echo "   export ROLLOUT_TP_SIZE=1"
echo "   export EXPERIMENT_NAME=countdown-qwen2.5-0.5b"
