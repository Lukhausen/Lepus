...
echo "[5/6] Downloading Qwen2.5-14B model (if needed)..."
mkdir -p models/
# Check if model directory exists before cloning
if [ ! -d "models/Qwen2.5-14B/.git" ]; then # Check for .git to be more robust
    cd models/
    echo "    Cloning model repo (this uses LFS and may take time)..."
    # Clone the model - Git LFS is essential here
    git lfs clone https://huggingface.co/Qwen/Qwen2.5-14B # Keep output for progress
    cd ../ # Go back to TinyZero project root
    echo "    Model download complete."
else
    echo "    Model directory models/Qwen2.5-14B already exists."
fi
...
echo "   export BASE_MODEL=./models/Qwen2.5-14B"
echo "   export DATA_DIR=./data/arc_agi_two"
echo "   export ROLLOUT_TP_SIZE=1"
echo "   export EXPERIMENT_NAME=arc_agi_two-qwen2.5-14B"
